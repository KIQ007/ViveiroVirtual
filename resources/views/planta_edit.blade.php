...
<div class="min-h-screen text-green-900 m-0 p-0 bg-center bg-cover flex items-center justify-center" 
  ...
  <div class="max-w-lg w-full mx-4 my-4 p-6 bg-white/30 border-2 border-gray-300/30 rounded-xl shadow-md backdrop-blur-xl">
    ...
    <div class="mb-6 relative">
      ...
      <h1 class="text-2xl font-bold text-green-900 text-center">🌿 Editar Planta</h1>
    </div>
    ...
    <form action="{{ route('plantas.update', ['planta' => $planta->id]) }}" method="POST" enctype="multipart/form-data" id="formPlanta" class="space-y-4">
      @csrf
      @method('PUT')
      ...
      <div>
        ...
        <input type="text" name="nome" required
          ...
          placeholder="Ex: Manjericão, Sálvia, Hortelã" value="{{ old('nome', $planta->nome) }}">
      </div>
      ...
      <div>
        ...
        <input type="text" name="nome_cientifico"
          class="w-full px-3 py-2 border border-green-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
          placeholder="Ex: Ocimum basilicum (opcional)" value="{{ old('nome_cientifico', $planta->nome_cientifico) }}">
      </div>
      ...
      <div>
        ...
        <textarea name="descricao" rows="3"
          class="w-full px-3 py-2 border border-green-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
          placeholder="Alguma informação adicional sobre a planta...">{{ old('descricao', $planta->descricao) }}</textarea>
      </div>
      ...
      <div>
        ...
        <div class="border-2 border-dashed border-green-300 rounded-lg p-4 text-center transition hover:border-green-400 bg-white text-green-800">
          ...
          <div id="previewContainer" class="mt-3">
            <img id="imagePreview" 
                 src="{{ $planta->foto ? asset('storage/' . $planta->foto) : '#' }}" 
                 alt="Pré-visualização" 
                 class="mx-auto max-h-40 rounded-md border border-green-200"
                 @if(!$planta->foto) style="display:none" @endif>
  ...
  document.getElementById('formPlanta').addEventListener('submit', async function (e) {
    ...
    try {
      const response = await fetch(url, {
        method: 'POST', // @method('PUT') será interpretado corretamente no Laravel
        headers: {
      ...
      if (response.ok) {
        ...
        alert('Erro ao atualizar: ' + (data.message || 'Verifique os campos.'));
      }
    } catch (error) {
