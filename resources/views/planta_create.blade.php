...
@section('content')
  <div class="max-w-lg w-full mx-4 my-4 p-6 bg-white/30 border-2 border-gray-300/30 rounded-xl shadow-md backdrop-blur-xl">
    ...
    <form action="{{ route('plantas.store') }}" method="POST" enctype="multipart/form-data" id="formPlanta" class="space-y-4">
      @csrf
      ...
      <div>
        ...
        <div class="border-2 border-dashed border-green-300 rounded-lg p-4 text-center transition hover:border-green-400 bg-white text-green-800">
          ...
          <div id="previewContainer" class="hidden mt-3">
            <img id="imagePreview" src="#" alt="Pré-visualização" class="mx-auto max-h-40 rounded-md border border-green-200">
            <button type="button" onclick="removeImage()" class="mt-2 text-sm text-red-600 hover:text-red-800">Remover imagem</button>
          </div>
